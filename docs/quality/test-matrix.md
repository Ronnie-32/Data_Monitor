...
| AUT-TT-07 | Automated | 10,12 | Display range | Use configured periods. | Range begins period1.start and ends period8.end. | tests/unit/services/test_timetable_service.py |  |
...
| AUT-TT-12 | Automated | 10,12 | Unconfigured periods | Build timetable without complete period set. | ViewModel returns configuration-required state and no invented vertical bounds. | tests/unit/services/test_timetable_service.py |  |
...
| AUT-PROFILE-06 | Automated | 13,14,23 | Global-data isolation | Switch Profiles after storing Todo/cities/finance/class periods/network data. | Global data remains unchanged. | tests/unit/services/test_profile_service.py |  |
...
| AUT-PROFILE-16 | Automated | 13,14,23 | Theme list | Enumerate supported themes. | Exactly four approved light themes; value persists per Profile. | tests/unit/services/test_profile_service.py |  |
| AUT-LAYOUT-01 | Automated | 14 | Grid topology | Round-trip a 12-column GridStack layout through save/load and outer-window resize state. | x/y/w/h topology remains unchanged; only pixel dimensions are a Web runtime concern. | tests/unit/presentation/test_layout_state.py |  |
...
| AUT-FIN-08 | Automated | 18,22 | Minimal display | Present finance item. | ViewModel contains name/value/change where meaningful and optional secondary label; no chart/history payload. | tests/unit/services/test_finance_service.py or tests/unit/presentation/test_finance_presenter.py |  |
...
| AUT-FX-04 | Automated | 19 | FX presentation | Present USD/EUR/JPY/HKD normalized values. | All follow same 1 foreign unit = CNY display convention. | tests/unit/providers/fx/test_parser.py |  |
...
| REV-UI-01 | Review | 23 | Visual exclusions | Inspect CSS/assets. | No acrylic/mica, dark theme, continuous animation, runtime CDN. |  |  |
...
| MAN-LAYOUT-01 | Manual | 14 | Layout Edit | Enter from Settings. | Dashboard foreground/editable; GridStack + whole-window move/resize enabled; Todo interaction disabled; Save/Cancel visible. | Screenshot / short note / command output as appropriate |  |
...
| MAN-TT-03 | Manual | 12 | Continuous positioning | Compare events at different arbitrary clock times. | Vertical positions reflect real continuous time between period1 start and period8 end. | Screenshot / short note / command output as appropriate |  |
| MAN-TT-04 | Manual | 12,25 | Unconfigured periods | Use fresh/unconfigured period settings and open timetable. | Compact configure-first empty state appears; no fake period times; Today Agenda still works. | Screenshot / short note / command output as appropriate |  |
...
| MAN-UI-03 | Manual | 23 | Themes | Switch all four themes/Profile combinations. | All four light themes are distinct, readable, and persist per Profile. | Screenshot / short note / command output as appropriate |  |
...
| MAN-SET-05 | Manual | 25 | Class periods | Attempt partial/duplicate/invalid periods then save valid 1..8. | Invalid set is not active; complete valid set saves; no school times invented. | Screenshot / short note / command output as appropriate |  |
